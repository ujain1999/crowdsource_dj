...
async def websocket_endpoint(
    ...
    try:
        ...
        while True:
            ...
            print(f"[WS] Received message type: {message_type} from {username} in room {room_code}")
            ...
            if message_type == "play":
                ...
                # Update state
                manager.set_playback_state(room_code, current_song_id, True, current_time)
                ...
                # Update state
                manager.set_playback_state(room_code, state.get("current_song_id"), False, current_time)
                # Broadcast to all users
                await manager.broadcast(room_code, {
                    "type": "pause",
                    "current_time": current_time
                ...
                # Update state
                manager.set_playback_state(room_code, state.get("current_song_id"), is_playing, current_time)
                # Broadcast to all users
                await manager.broadcast(room_code, {
                    "type": "seek",
                    "current_time": current_time
